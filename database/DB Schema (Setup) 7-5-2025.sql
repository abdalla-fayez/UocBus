...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Trip_Creation` ()  MODIFIES SQL DATA COMMENT 'Trip Generation Function' BEGIN
    ...
    SELECT
        ...
        r.id                  AS route_id,
        DATE_ADD(CURDATE(),
                 INTERVAL off.offset DAY) AS trip_date,
        b.vacant_seats        AS available_seats,
        r.route_name          AS route_name,
        r.trip_type           AS trip_type,
        r.time                AS trip_time
    FROM routes AS r
    JOIN buses  AS b ON b.id = r.bus_id
    -- Build offsets 1..7, pick only those that fall on Sun–Thu, take the first two
    CROSS JOIN (
        SELECT ds.offset
        FROM (
        ...
        WHERE DAYOFWEEK(DATE_ADD(CURDATE(),
                                 INTERVAL ds.offset DAY))
              NOT IN (6,7)    -- exclude Friday (6) + Saturday (7)
        ORDER BY ds.offset   -- earliest dates first
        LIMIT 2              -- pick the next two days
    ) AS off
    WHERE
        r.status = 'Active'
      AND NOT EXISTS (
        SELECT 1
         ...
         WHERE t.bus_id    = r.bus_id
           AND t.route_id  = r.id
           AND t.trip_date = DATE_ADD(CURDATE(),
                                      INTERVAL off.offset DAY)
      );
END$$
...
CREATE TABLE IF NOT EXISTS `pickup_points` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_route_point` (`route_id`,`name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
--
-- RELATIONSHIPS FOR TABLE `system_config`:
--
...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `tickets_booked` int(11) NOT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
